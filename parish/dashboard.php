...
$breadcrumb = 'Parish Dashboard';

// Get statistics data
try {
    // Get total members count
    $members_stmt = $conn->prepare("SELECT COUNT(*) as total FROM members WHERE parish_id = ?");
    $members_stmt->bind_param("i", $parish_id);
    $members_stmt->execute();
    $members_count = $members_stmt->get_result()->fetch_assoc()['total'];
    $members_stmt->close();

    // Get communicants count
    $communicants_stmt = $conn->prepare("SELECT COUNT(*) as total FROM members WHERE parish_id = ? AND is_communicant = 1");
    $communicants_stmt->bind_param("i", $parish_id);
    ...
    $confirmed_stmt = $conn->prepare("SELECT COUNT(*) as total FROM members WHERE parish_id = ? AND nlc IS NOT NULL AND nlc != ''");
    $confirmed_stmt->bind_param("i", $parish_id);
    ...
    $baptized_stmt = $conn->prepare("SELECT COUNT(*) as total FROM members WHERE parish_id = ? AND baptism_date IS NOT NULL");
    $baptized_stmt->bind_param("i", $parish_id);
    $baptized_stmt->execute();
    $baptized_count = $baptized_stmt->get_result()->fetch_assoc()['total'];
    $baptized_stmt->close();

    // Get gallery items count
    $gallery_stmt = $conn->prepare("SELECT COUNT(*) as total FROM gallery WHERE parish_id = ?");
    $gallery_stmt->bind_param("i", $parish_id);
    $gallery_stmt->execute();
    $gallery_count = $gallery_stmt->get_result()->fetch_assoc()['total'];
    $gallery_stmt->close();

    // Get recent members
    $recent_members_stmt = $conn->prepare("SELECT * FROM members WHERE parish_id = ? ORDER BY created_at DESC LIMIT 5");
    $recent_members_stmt->bind_param("i", $parish_id);
    ...
    $recent_gallery_stmt = $conn->prepare("SELECT * FROM gallery WHERE parish_id = ? ORDER BY uploaded_at DESC LIMIT 4");
    $recent_gallery_stmt->bind_param("i", $parish_id);
    $recent_gallery_stmt->execute();
    $recent_gallery = $recent_gallery_stmt->get_result();
    $recent_gallery_stmt->close();

} catch (Exception $e) {
    error_log("Parish dashboard error: " . $e->getMessage());
    die("Error loading parish dashboard data");
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?php echo htmlspecialchars($parish_name); ?> Dashboard - Keta-Akatsi Diocese</title>
...
<body>
    ...
    <div class="dashboard-container">
        <h1 class="dashboard-header">
            <i class="fas fa-church"></i> <?php echo htmlspecialchars($parish_name); ?> Dashboard
        </h1>
        
        <div class="stats-grid">
            <div class="stat-card">
                <h3 class="stat-title"><i class="fas fa-users" style="color: var(--primary-color);"></i> Total Members</h3>
            ...
            <div class="stat-card">
                ...
                <a href="members.php?filter=baptized" class="stat-link">
                    View Baptised <i class="fas fa-arrow-right"></i>
                </a>
            </div>
        ...
        <div class="recent-section">
            <div class="recent-card">
                <div class="recent-title">
                    <span><i class="fas fa-user-clock"></i> Recently Added Members</span>
                    <a href="actions/member-crud.php?action=add" class="add-btn">
